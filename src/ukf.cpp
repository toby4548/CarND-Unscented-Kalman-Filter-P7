...
UKF::UKF() {
  ...
  std_a_ = 2;  //original:30  best: 2

  // Process noise standard deviation yaw acceleration in rad/s^2
  std_yawdd_ = 0.3;  //original:30  best:0.3
  ...
  */
  ...
  if(is_initialized_) {
    ...
    if (meas_package.sensor_type_ == MeasurementPackage::LASER) {
      //convert radat from polar to cartesian coordinates
...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  for (int i=0;i < n_aug_;i++) {
    Xsig_aug.col(i+1) = x_aug + root.col(i);
    Xsig_aug.col(i+1+n_aug_) = x_aug - root.col(i);
  }
